...
def register_task_type(tasks: Dict[str, str]):
    ...
    for tasks_name, tasks_value in tasks.items():
        if tasks_name.lower() in RESERVED_TASK_NAMES or \
                tasks_name.upper() in RESERVED_TASK_NAMES or \
                tasks_value.lower() in RESERVED_TASK_NAMES or \
                tasks_value.upper() in RESERVED_TASK_NAMES:
            raise RegisterFailure(f"Encountered potential conflicts "
