...
def target_logp_from_policy_and_act(policy, obs, act, rnn_states, masks):
    if obs.ndim == 4: # [B, T, N, *]
        B, T, N, _ = obs.shape
        _cast = lambda x: torch.tensor(x.reshape((1, B*T*N, -1)), 
    ...
    act = torch.tensor(act.squeeze(-1), dtype=torch.float32,
...
def compute_vtrace(
    policy, obs, reward, value, done,
    rnn_state, act, act_dist, gamma,
    clip_rho_threshold, clip_pg_rho_threshold
):
    behavior_logp = behavior_logp_from_prob_and_act(act_dist, act)
    target_logp = target_logp_from_policy_and_act(
        policy, obs, act, rnn_state, done)
    ...
    vs, pg_adv = vtrace_return(
        log_rhos, gamma, rewards, 
        values, bootstrap_values, dones,
        clip_rho_threshold, clip_pg_rho_threshold)
...
def vtrace_return(
    ...
    assert values.ndim == rho_rank and rewards.ndim == rho_rank, \
        (log_rhos.shape, values.shape, rewards.shape)
    ...
    vs_tp1 = np.concatenate(
        [vs[1:], np.expand_dims(bootstrap_values, 0)], axis=0)
    
    clipped_pg_rhos = np.minimum(clip_pg_rho_threshold, rhos)

    pg_advantages = clipped_pg_rhos * (rewards + (1 - dones) * discounts * vs_tp1 - values)
